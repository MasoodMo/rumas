<?php
	...
	if(isset($_POST['upload_telugu'])){
		...
		if(isset($_FILES['audio_file'])){
			...
			move_uploaded_file($file_tmp,"songs/telugu_songs/".$file_name);
         }
         ...
         if(isset($_FILES['song_image'])){
			...
			move_uploaded_file($file_tmp,"songs/telugu_songs/img/".$file_name);
		 }

		$sql = "INSERT INTO telugu_albums(`song_name`,`song_format`,`singer_name`,`movie_name`,`song_image`,`audio_file`) VALUES('$song_name','$song_format','$singer_name','$movie_name','$song_image','$audio_file')";
		$result = mysqli_query($conn,$sql);
		if($result){
			...
			echo '<script type="text/javascript">';
            echo 'setTimeout(function () { sweetAlert("Oops..."," Error while uploading.Please check your internet connection!","error");';
            echo '}, 500);</script>';
		}
    ...
    $sql = "SELECT * FROM telugu_albums ORDER BY song_id ASC";
	$result = mysqli_query($conn,$sql);
	...
	while($rows = mysqli_fetch_array($result)){
		...
		if(isset($_POST[$song_id])){
			...
			$sql = "SELECT * FROM telugu_albums WHERE song_id = '$song_id' ";
			...
			$sql = "DELETE FROM telugu_albums WHERE song_name = '$song_name' ";
			$results = mysqli_query($conn,$sql);

			if($results){
				echo '<script type="text/javascript">';
                echo 'setTimeout(function () { sweetAlert("Deleted"," <b>Song '.$song_name.' is deleted from telugu albums</b>","success");';
				echo '}, 500);</script>';
			}else{
				echo '<script type="text/javascript">';
...
?>
    ...
    <div class="alert alert-primary" role="alert">
         <center><b>Telugu Songs</b></center>
    </div>
	...
	<div class="form-row">
		...
		<div class="col">
			<div class="text-center">
				<button type="submit" name="upload_telugu" class="btn btn-default btn-rounded mb-4">Upload <i class="fa fa-upload"></i></button>
			</div>
		</div>
	</div>	
</form>
...
<?php 
    ...
    $sql = "SELECT * FROM telugu_albums order by song_id";
    $result = mysqli_query($conn,$sql);

    echo "
        <div class='card'>
            <h3 class='card-header text-center font-weight-bold text-uppercase py-4'>Telugu Songs</h3>
            <div class='card-body'>
                <div id='table' class='table-editable'>
                    <table class='table table-bordered table-responsive-md table-striped text-center'>
                        <tr>
...
?>
	...
	<div class="cpy-right text-center">
		<p>©  2022 RuMas Music Player. All rights reserved</p>
	</div>
    <!-- //copyright -->
    <!-- js-->
